...
* **Channel name**: `R<band>-<network>-<district> <city> <call>` (e.g. `R2M-BR-6 Nacka SM0ABC`).
  ...
  * Network codes: BrandMeister→BR, Wires-X→WX, SvxReflector/SvxLink→SV, Echolink→EL, IRLP→IR, empty→FM, otherwise the uppercase value.
...
* **Access tones**
  * RX tone is always `None`.
  * TX tone is the first numeric CTCSS value (40–300 Hz) found in `access` (e.g. `1750 / 107.2` → `107.2Hz`); if none is found, `None` is written. Tone bursts (e.g. 1750) and DTMF are ignored.
...
* CTCSS handling that populates TX tone when a numeric 40–300 Hz tone is present, otherwise `None`; RX tone is always `None`.
